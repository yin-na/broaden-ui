  ...
  <div class="layout">
    <div class="content">
      <div class="content-left">
        ...
        <mu-paper>
          <el-menu v-for="tool in tools" :key="tool.name">
            <el-menu-item  @click="drawType(tool)" :class="{'selected':tool.ischoose}">
              <template>
                  <img :src="tool.icon" alt="" style="margin-right:10px">
                  <span>{{tool.name}}</span>
              </template>
            </el-menu-item>
          </el-menu>
        ...
        <div class="nav">
        <el-button v-for="tab in tabs" :key="tab.name" class="tab demo-flat-button" style="border:none;font-size:15px" :icon="tab.icon" @click="tabfun(tab.fun)"  primary backgroundColor="#FFFFFF">
          <template>
            <img :src="tab.icon" alt="" style="margin-right:10px;width:16px">
            <span>{{tab.name}}</span>
        </template>
        </el-button>
        <el-button  class="tab demo-flat-button" @click="setColor()"  primary backgroundColor="#FFFFFF" :color="color.hex" style="border:none;font-size:15px">
          <template>
            <img src="@/assets/user/Drawing.png" alt="" style="margin-right:10px;width:20px">
            <span>{{chooseColorBtn}}</span>
          </template>
        </el-button>
      ...
      <PhotoshopPicker v-model="color" v-if="ischoosecolor" style="position:absolute;z-index:10;top:56px;" @ok="setColor()" @cancel="falseColor()"/>
        <el-button class='btn' type="primary" @click="dialogshow">提交当前绘画结果</el-button>
      </div>
      <div class="content-right">
        <div class="body" :style="{width:canvasSize.width + 'px',height: canvasSize.height + 'px' }">
           <div class="fwb" style="color:#FF0000;font-size:16px;font-weight:bold;margin-bottom:20px;padding-left:3%">绘画：请在下面画出房子、树木和完整的人物</div>
            <canvas id="canvas" ref="canvas" :style="{cursor:curcursor}"></canvas>
            <canvas id="canvas_bak" ref="canvas_bak" :style="{cursor:curcursor}" style='border:1px dashed #7A90AF'></canvas>
        </div>
      </div>
    </div>
    <el-dialog
      ...
      :before-close="handleClose">    
          <el-input v-model="picinput" placeholder="请输入内容"></el-input>
        <span slot="footer" class="dialog-footer">
          <el-button @click="dialogVisible = false">取 消</el-button>
          <el-button type="primary" @click="gotoResult">确 定</el-button>
        </span>
    </el-dialog>
  </div>
</template>
...
export default {
  ...
  data() {
    return {
      ...
      tools: [
        {
          name: '铅笔',
          icon: require("@/assets/user/pencli.png"),
          fun: 'pencil',
          ischoose: false,
        },
        {
          name: '直线',
          icon: require("@/assets/user/xian.png"),
          fun: 'line',
          ischoose: false,
        },
        {
          name: '圆形',
          icon: require("@/assets/user/yuan.png"),
          fun: 'circle',
          ischoose: false,
        },
        {
          name: '矩形',
          icon: require("@/assets/user/juxing.png"),
          fun: 'square',
          ischoose: false,
        },
        {
          name: '涂鸦',
          icon: require("@/assets/user/maobi.png"),
          fun: 'handwriting',
          ischoose: false,
        },
        {
          name: '橡皮',
          icon: require("@/assets/user/rubber.png"),
          fun: 'rubber',
          ischoose: false,
        },
      ],
      tabs: [
        {
          name: '清除',
          icon: require("@/assets/user/delete.png"),
          fun: 'clear',
        },
        // {
        //   name: '保存',
  ...
  methods: {
    ...
    downloadImage() {
      console.log(this.canvas.toDataURL())  //保存的图片路径
  ...
  mounted() {
    ...
    window.addEventListener('resize', () => {
      this.canvasSize = {
        width: window.screen.availWidth - 1200,
        height: window.screen.availHeight * 0.75,
      }
    })
  },
