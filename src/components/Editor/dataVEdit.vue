    ...
    <div class="panel" style="background: #fff">
        ...
        <div class="config-box">
            ...
            <div v-if="currentElement.data.datacon.type == 'raw'">
              ...
              <!-- 图表数据 编辑  -->
              <div v-if="currentElement.data.settings.type != 'water_level'">
                <vue-scroll :ops="ops" style="width:100%;height: 350px;">
                <el-table
                  :data="currentElement.data.generated.data"
                  :header-cell-style="{fontSize:'14px',height:'30px'}"
                  :row-style="{height:'30px'}"
                  style="width: 100%;border-color: #999;font-size: 14px;">
                  <el-table-column
                    v-for="(item) in Object.keys(currentElement.data.generated.data[0])"
                    :key="item"
                    :label="item"
                    >
                    {{item}}
                    <template slot-scope="scope">
                      <input class="input-table" v-model="scope.row[item]" @change="handleResetData"/>
                    </template>
                  </el-table-column>
                  <el-table-column label="操作" align="center">
                    <template slot-scope="scope">
                      <span style="color:#2c75ef;font-size: 12px;cursor: pointer;text-align: center;display: block;" @click="(currentElement.data.generated.data).splice(scope.$index,1)">删除行</span>
                    </template>
                  </el-table-column>
                </el-table>
              ...
              <div v-else>
                 请输入数字：<input class="input-table" v-model="currentElement.data.generated.data[0]" @change="handleResetData"/>
              </div>
...
  },
  ...
  methods: {
    ...
    //  20201014  duziqiang  增加  fix 组件数据修改后，组件不改变的bug
    // datav  修改完数据需要重新渲染，否则图表不会改变    issue 136  https://github.com/DataV-Team/DataV/issues/136
    //  http://datav.jiaminghi.com/guide/#%E7%94%A8%E5%89%8D%E5%BF%85%E7%9C%8B   状态更新
    /**
     * 组件props均未设置deep监听，刷新props时，请直接生成新的props对象（基础数据类型除外），或完成赋值操作后使用ES6拓展运算符生成新的props对象（this.someProps = { ...this.someProps }）。
     */
    handleResetData(){
        if(this.currentElement.data.settings.type!='water_level'){
          for(let i in this.currentElement.data.generated.data){
            this.currentElement.data.generated.data[i].value=parseInt(this.currentElement.data.generated.data[i].value)
          }
        }else{
           this.currentElement.data.generated.data[0]=parseInt(this.currentElement.data.generated.data[0])
        }
        
         this.currentElement.data.generated={ ...this.currentElement.data.generated }
         console.log(   this.currentElement.data.generated)
    },
