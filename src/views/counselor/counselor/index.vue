            ...
            <el-col :xs="24" :sm="24" :md="24" :lg="24" :xl="24">
                <el-card class="box-card" style="box-shadow: none;border: none">
                    ...
                    <el-table v-loading="loading" ref="scaleTable" :data="data" size="mini" :height="tableHeight"
                        ...
                        <el-table-column show-overflow-tooltip align="center" prop="user.username"
                                         label="用户名" width="100px"/>
                        <!--<el-table-column show-overflow-tooltip align="center" prop="dictValue.label"
                                         label="咨询师等级"/>-->
                        <el-table-column show-overflow-tooltip align="center" prop="user.sex" label="性别" width="60px">
                            <template slot-scope="scope">
                                <div v-if="scope.row.user.sex==='2'">
                                    未知
                                </div>
                                <div v-if="scope.row.user.sex==='0'">
                                    女
                                </div>
                                <div v-if="scope.row.user.sex==='1'">
                                    男
                                </div>
                                <!--{{scope.row.user.sex===0?'男':'女'}}-->
                            </template>
                        </el-table-column>
                        <el-table-column show-overflow-tooltip align="center" prop="user.mobilePhone"
                                         label="移动电话(登录账号)"/>
                        <el-table-column show-overflow-tooltip align="center" prop="user.email"
                                         label="邮箱"/>
                        <el-table-column show-overflow-tooltip align="center" prop="city"
                                         label="地址">
                            <template slot-scope="scope" v-if="scope.row.city">
                                {{scope.row.city.cname}},{{scope.row.city.children[0].cname}},{{scope.row.city.children[0].children[0].cname}}
                            </template>
                        </el-table-column>
                        <el-table-column show-overflow-tooltip align="center" prop="aptitudeType"
                                         label="认证资质">
                            <!--<template slot-scope="scope">
                                <span v-for="(item,index) in scope.row.aptitudeTypeList">
                                    {{index ===0 ? item.label:',' + item.label}}
                                </span>
                            </template>-->
                        </el-table-column>
                        <el-table-column show-overflow-tooltip align="center" prop="labelList"
                                         label="心理困惑标签">
                            <template slot-scope="scope">
                                <span v-for="(item,index) in scope.row.labelList">
                                    {{index ===0 ? item.name:',' + item.name}}
                                </span>
                            </template>
                        </el-table-column>
                        <el-table-column show-overflow-tooltip align="center" prop="user.avatar" label="头像">
                            <template slot-scope="scope">
                                <el-image :src="scope.row.user.avatar" style="width: 30px;height: 30px; margin-top: 2px">
...
<script>
    ...
    import {getAllLabel , del} from '@/api/intervention/counselor.js'
    import { list } from '@/api/counselor/counselor.js'
    ...
    export default {
        ...
        methods: {
            ...
            // 新增
            handleCreate () {
                this.$router.push("/counselor/counselorOption/" + -1 + '/create');
            },
            handleView(row) {
                this.$router.push("/counselor/counselorOption/" + row.id + '/view');
            },
            handleUpdate(row) {
                this.$router.push("/counselor/counselorOption/" + row.id + '/update');
            },
            handleDelete(id) {
