        ...
        <el-card class="box-card" style="border: none;box-shadow: none">
          <div class="contain-tit">
            ...
            <div class="fr">
              ...
              <guide-into :type='"学生信息导入模板"' :templateUrl='"student/downloadTemplate"' :importUrl='"student/uploadAnswer"' @success='successBtn'></guide-into>
              ...
              <el-button size="mini" type="primary" @click="saoCode()">扫码注册普查</el-button>
              <el-button size="mini" type="danger" icon="el-icon-delete" @click="batchDelete">批量删除</el-button>
            </div>
          </div>
          <div class="head-container">
            <el-input v-model="params.loginName" placeholder="学籍号" style="width: 150px;margin-right:10px"></el-input>
            <el-input v-model="params.name" placeholder="姓名" style="width: 150px;margin-right:10px"></el-input>
            <el-select @change="grades"  placeholder="请选择年级" style="width: 150px;margin-right:10px" v-model="gradeId">
                            <el-option
             ...
             <el-select placeholder="请选择班级（请先选择年级）" style="width: 200px;margin-right:10px" v-model="params.classId">
                            <el-option
          ...
          >
            ...
            <el-table-column  align="center" prop="loginName" label="学籍号（登录账户）" />
...
import { getStudentList,getClass, del, getSchool } from "@/api/student/student";
import {resetPassword} from '@/api/base/user'
// import GuideInto from './components/guideInto'
...
export default {
  ...
  methods: {
    ...
    // 新增
    handleCreate() {
      this.$router.push("/education/userManer/studentOption/" + -1 + '/create');
    },
    handleView(row) {
       this.$router.push("/education/userManer/studentOption/" + row.userId + '/read');
    },
    handleUpdate(row) {
      this.$router.push("/education/userManer/studentOption/" + row.userId + '/update');
    },
    // 扫码普查跳转
    saoCode () {
      this.$router.push("/education/userManer/scanCode")
    },
    // 重置密码
    handleResetPass(row){
